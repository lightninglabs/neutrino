...
func (f *FileHeaderImportSource) GetHeaderMetadata() (*HeaderMetadata, error) {
	...
	// Extract start height. First check if the value is negative by
	// examining the most significant bit (sign bit in two's complement). If
	// the MSB is set, it indicates a negative value which would be
	// interpreted as a large positive value
	// (between 0x80000000 and 0xFFFFFFFF) when read as uint32, which is
	// invalid in this context.
	startHeightRaw := buf[startHeightOffset:HeaderMetadataSize]
	if len(startHeightRaw) == 4 && (startHeightRaw[3]&0x80) != 0 {
		// Get negative value.
		var negativeValue int32
		binary.Read(
			bytes.NewReader(startHeightRaw), binary.LittleEndian,
			&negativeValue,
		)
		return nil, fmt.Errorf("invalid negative value detected for "+
			"StartHeight: %d", negativeValue)
	}
	startHeight := binary.LittleEndian.Uint32(startHeightRaw)
...
func (f *FileHeaderImportSource) GetHeader(index int) (Header, error) {
	...
	offset := HeaderMetadataSize + (index * f.metadata.HeaderSize)
...
func (s *HeadersImport) Import(ctx context.Context) (*ImportResult, error) {
	...
	// strategy/strategies for handling divergence region in the target
	// header stores while importing.
...
func (s *HeadersImport) determineProcessingRegions() (*ProcessingRegions, error) {
	...
	// tip height. Using the default AppendOnly mode, these headers will be
	// skipped to preserve existing data. These headers need to be processed
	// according to the selected OverlapMode.
...
func (s *HeadersImport) writeHeadersToTargetStores(
	...
	if err := s.options.TargetFilterHeaderStore.WriteHeaders(
		...
		blkHdrsToTruncate := uint32(len(blockHeaders))
		blockHeadersToTruncate := blkHdrsToTruncate
		...
		if rollbackErr != nil {
			return fmt.Errorf("failed to "+
				"rollback %d headers from "+
				"target block header store "+
				"after filter header write "+
				"failure. Block error: %w, "+
				"filter error: %v",
				blockHeadersToTruncate,
				rollbackErr, err)
		}
