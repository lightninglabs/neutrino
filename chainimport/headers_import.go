...
const (
	...
	// size is provided.
	DEFAULT_WRITE_BATCH_SIZE_PER_REGION = 16384
)
	...
	if err := s.options.TargetFilterHeaderStore.WriteHeaders(
		...
		if rollbackErr != nil {
			return fmt.Errorf("failed to rollback %d headers from "+
				...
				"header write failure. Block error: %w, "+
...
func (options *ImportOptions) Import(
	...
	if options.WriteBatchSizePerRegion <= 0 {
		batchSize := DEFAULT_WRITE_BATCH_SIZE_PER_REGION
		options.WriteBatchSizePerRegion = batchSize
...
func (options *ImportOptions) createBlkHdrImportSrc() HeaderImportSource {
	return &FileHeaderImportSource{
		URI:           options.BlockHeadersSource,
...
func (options *ImportOptions) createFilterHdrImportSrc() HeaderImportSource {
	return &FileHeaderImportSource{
		URI:           options.FilterHeadersSource,
...
// GetDefaultWriteBatchSize returns the default write batch size for the
// import operation.
func GetDefaultWriteBatchSize() int {
	minBatchSize, maxBatchSize := 1024, 16384
	return adaptiveWriteBatchSize(minBatchSize, maxBatchSize)
}

// adaptiveWriteBatchSize determines an optimal batch size based on available
// system memory. It returns a batch size between minBatchSize and maxBatchSize.
func adaptiveWriteBatchSize(minBatchSize, maxBatchSize int) int {
	...
	availableMemMB := int64(0)
	if err == nil {
		// On Linux: use syscall data.
		availableMemB := int64(info.Freeram) * int64(info.Unit)
		availableMemMB = availableMemB / (1024 * 1024)
	} else {
		// Fallback: estimate using Go runtime.
		availableMemB := int64(memStats.Sys - memStats.HeapInuse)
		availableMemMB = availableMemB / (1024 * 1024)
	}
	// Define memory thresholds in MB for different batch sizes based on
	// benchmark results showing peak memory usage for each batch size.
	switch {
	case availableMemMB < 30:
		// 1024: Peak ~13.4MB.
		return minBatchSize
	case availableMemMB < 50:
		// Peak: ~18.8MB.
		return 2048
	case availableMemMB < 80:
		// Peak: ~31.9MB.
		return 4096
	case availableMemMB < 150:
		// Peak: ~42MB.
		return 8192
	default:
		// 16384: Peak ~66MB (optimal performance).
		return maxBatchSize
	}
}
