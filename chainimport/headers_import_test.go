...
// TestImport tests the initialization, validation, and execution logic for
// ImportOptions and the import process. It covers error and success scenarios
// for header source requirements, option validation, and import operations.
func TestImport(t *testing.T) {
	...
	t.Run("ImportOperation/Skip", func(t *testing.T) {
		t.Parallel()
		type Prep struct {
			options *ImportOptions
		}
		...
		testCases := []struct {
		...
		}{
			{
				...
				prep: func() Prep {
					...
					tFS := &mockFilterHeaderStore{}
					tFS.On("ChainTip").Return(
						&chainhash.Hash{}, uint32(3),
						nil,
					)
				...
				verify: func(v Verify) {
					...
					b := DEFAULT_WRITE_BATCH_SIZE_PER_REGION
					ops := v.importOptions
					require.Equal(
						v.tc, b,
	...
	t.Run("ImportOperation/FileHeaderImportSource", func(t *testing.T) {
		t.Parallel()
		type Prep struct {
			options *ImportOptions
			cleanup func()
		...
		testCases := []struct {
		...
		}{
			{
				...
				prep: func() Prep {
					...
					// Close block headers import source
					// to be opened during import.
					bFile.Close()
					...
					// Close filter headers import source
					// to be opened during import.
					fFile.Close()
				...
				verify: func(v Verify) {
					// Verify the user write batch size is
					...
					// Verify headers added/processed
					// excluding the genesis header.
					require.Equal(
	...
	t.Run("ImportOperation/HTTPHeaderImportSource", func(t *testing.T) {
		t.Parallel()
		type Prep struct {
...
// TestImportOpenSources tests the opening and validation of header import
// sources, including file existence, metadata, and validator requirements. It
// covers error and success scenarios for opening sources and reading metadata.
func TestImportOpenSources(t *testing.T) {
	...
	t.Run("HTTPHeaderImportSource", func(t *testing.T) {
		t.Parallel()
		type Prep struct {
			hImport *HeadersImport
			cleanup func()
		...
		testCases := []struct {
		...
		}{
			{
				...
				expectErr: true,
				expectErrMsg: "failed to download block " +
					"headers",
			},
			{
				...
				expectErr: true,
				expectErrMsg: fmt.Sprintf("failed to download "+
					"file: status code %d",
					http.StatusNotFound),
			},
			{
				...
				prep: func() Prep {
					...
					// Mock open operation on block header
					// import source to focus solely on
					// testing HTTP header import.
				...
				expectErr: true,
				expectErrMsg: "failed to download filter " +
					"headers",
			},
			{
			...
			},
			{
				...
				prep: func() Prep {
					...
					// Mock open operation on filter header
					// import source to focus solely on
					// testing HTTP header import.
...
// TestHeaderMetadata tests the reading, validation, and storage of header
// metadata in import files. It covers error cases for file and metadata issues,
// and verifies correct metadata storage and retrieval.
...
// TestHeader tests header retrieval, iteration, and validation logic for both
// block and filter headers. It covers error and success scenarios for single
// and sequential header access, validation, and processing regions.
	...
	t.Run("Retrieval/SingleHeader", func(t *testing.T) {
		...
		for _, tc := range testCases {
			t.Run(tc.name, func(t *testing.T) {
				...
				header, err := prep.hISource.GetHeader(
					uint32(tc.index),
				)
				verify := Verify{
					tc:     t,
	...
	t.Run("Storage/NewHeadersRegion", func(t *testing.T) {
		...
		testCases := []struct {
			...
			},
			{
				...
				verify:    func(Verify) {},
				expectErr: true,
				expectErrMsg: "failed to read block header " +
			...
			},
			{
				...
				verify:    func(Verify) {},
				expectErr: true,
				expectErrMsg: "failed to read filter header " +
					"at height 1: I/O read error",
			},
			{
				...
				verify:    func(Verify) {},
				expectErr: true,
				expectErrMsg: "expected FilterHeader type, " +
					"got *chainimport.BlockHeader",
			},
			{
				...
				verify:    func(Verify) {},
				expectErr: true,
				expectErrMsg: "mismatch between block " +
					"headers (1) and filter headers (0) " +
					"for batch 1-100",
			},
			{
			...
			},
			{
				...
				prep: func() Prep {
					...
					// Setup mock block header store to
					// check if write error properly
					// propagated.
					b := &mockBlockHeaderStore{}
					in = mock.Anything
				...
				verify:    func(Verify) {},
				expectErr: true,
				expectErrMsg: "failed to write headers to " +
					"target stores",
			},
			{
				...
				prep: func() Prep {
					...
					// Configure import options. With write
					// batch size per region equal 1 it
					// would result in 4 batches of writing
					// to target stores.
					ops := &ImportOptions{
				...
				verify: func(v Verify) {
					// Assert that added/processed headers
					// count equal the prep test data
					// ignoring the genesis header
					require.Equal(
						v.tc, len(blockHdrs)-1,
						v.importResult.AddedCount,
					)
					...
					// Ensure no headers are skipped in the
					// new headers region.
					require.Equal(
						v.tc, 0,
						v.importResult.SkippedCount,
					...
					// Verify those added align with the
					// data inserted in the target stores.
					ops := v.importOptions
...
// TestImportAndTargetSourcesConstraintsSatisfaction tests compatibility and
// constraint checks between import and target sources. It covers error and
// success scenarios for metadata, type, network, and header count mismatches.
	...
	t.Run("Compatibility", func(t *testing.T) {
		...
		testCases := []struct {
			...
			},
			{
				...
				expectErr: true,
				expectErrMsg: "incorrect block header type: " +
					"expected BlockHeader, got " +
					"RegularFilterHeader",
			},
			{
				...
				expectErrMsg: "incorrect filter header type: " +
					"expected RegularFilterHeader, got " +
					"BlockHeader",
			},
			{
				...
				expectErr: true,
				expectErrMsg: fmt.Sprintf("network type "+
					"mismatch: block headers from "+
					"/path/to/blocks (%s), filter headers "+
					"from /path/to/filters (%s)",
			...
			},
			{
				...
				expectErr: true,
				expectErrMsg: fmt.Sprintf("network mismatch: "+
					"headers from import sources are for "+
					"%s, but target is %s", wire.SimNet,
					wire.MainNet),
			},
			{
				...
				expectErr: true,
				expectErrMsg: "start height mismatch: block " +
					"headers start at 1, filter headers " +
					"start at 3",
			},
			{
				...
				expectErr: true,
				expectErrMsg: "headers count mismatch: block " +
					"headers import source " +
					"/path/to/blocks (3), filter headers " +
					"import source /path/to/filters (5)",
			},
			{
				name: "ErrorOnNoHeadersFromImportSources",
				prep: func() *HeadersImport {
					// Mock block import store.
					bIS := &mockHeaderImportSource{}
					bT := headerfs.Block
				...
				expectErr:    true,
				expectErrMsg: "no headers available in sources",
			},
			{
	...
	t.Run("ChainContinuity", func(t *testing.T) {
		...
		testCases := []struct {
			...
			},
			{
				...
				expectErr: true,
				expectErrMsg: "failed to get target filter " +
					"header chain tip: I/O read error",
			},
			{
				...
				expectErr: true,
				expectErrMsg: "divergence detected between " +
					"target header store tip heights " +
					"(block=0, filter=2)",
			},
			{
				...
				expectErr: true,
				expectErrMsg: "target stores contain only " +
					"genesis block (height 0) but " +
					"import data starts at height 2, " +
					"creating a gap",
			},
			{
				...
				expectErr: true,
				expectErrMsg: "genesis header mismatch: " +
					"block header mismatch at height 0",
			},
			{
				...
				expectErr: true,
				expectErrMsg: "genesis header mismatch: " +
					"filter header mismatch at height 0",
			},
			{
			...
			},
			{
				...
				expectErr: true,
				expectErrMsg: "failed to get block header " +
					"from target at height 0: I/O read " +
					"error",
			},
			{
				...
				expectErr: true,
				expectErrMsg: "failed to get block header " +
					"from import source at height 1: I/O " +
					"read error",
			},
			{
				name: "ErrorOnTypeAssertingBlockHeader",
			...
			},
			{
				...
				expectErr: true,
				expectErrMsg: "failed to validate header " +
					"connection: header chain broken",
			},
			{
	...
	t.Run("HeadersVerification", func(t *testing.T) {
		...
		testCases := []struct {
			...
			},
			{
				...
				expectErr: true,
				expectErrMsg: "failed to get block header " +
					"from import source at height 101: " +
					"I/O read error",
			},
			{
				name:   "ErrorOnTypeAssertingBlockHeader",
			...
			},
			{
				...
				expectErr: true,
				expectErrMsg: "failed to get block header " +
					"from target at height 101: I/O read " +
					"error",
			},
			{
				...
				expectErr: true,
				expectErrMsg: "block header mismatch at " +
					"height 101",
			},
			{
			...
			},
			{
				...
				expectErr: true,
				expectErrMsg: "filter header mismatch at " +
					"height 101",
			},
			{
...
// Mocks a block header store for testing header store interactions.
...
// mockFilterHeaderStore mocks a filter header store for testing filter header
// store interactions.
