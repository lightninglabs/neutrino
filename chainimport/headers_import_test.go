...
func TestImport(t *testing.T) {
		...
		testCases := []struct {
		...
		}{
			{
				...
				prep: func() Prep {
					...
					// Setup import header stores.

					// Creat block headers import source.
					...
					fFile, c4, err := setupFileWithHdrs(
				...
				verify: func(v Verify) {
					// Verify the batch size is not set to
					// the default value.
					...
					// Verify headers added excluding the
					// genesis header
					require.Equal(
					...
					// Verify Start/End heights.
					require.Equal(
						v.tc, uint32(0),
						v.importResult.StartHeight,
					)
					require.Equal(
						v.tc, uint32(4),
						v.importResult.EndHeight,
					)
				},
			},
		}
	...
	t.Run("ImportOperation", func(t *testing.T) {
		t.Parallel()
		type Prep struct {
			options *ImportOptions
		}
