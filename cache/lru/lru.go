...
type Cache struct {
	// capacity represents how much this cache can hold. It could be number of
	// elements or a number of bytes, decided by the cache.Value's Size.
	capacity uint64

	// size represents the size of all the elements currenty in the cache.
	size uint64

	// ll is a doubly linked list which keeps track of recency of used elements
...
func (c *Cache) evict(needed uint64) error {
	if needed > c.capacity {
		return fmt.Errorf(
			"can't evict %v elements in size, since capacity is %v",
			needed, c.capacity)
	}
	...
	for room < needed {
		...
		if c.ll.Len() == 0 {
			...
			return fmt.Errorf(
				"all elements got evicted, yet still need to evict %v,"+
					"likelihood of error during size calculation",
				needed-room)
		}
		...
		if elr := c.ll.Back(); elr != nil {
			...
			if err != nil {
				return fmt.Errorf(
					"couldn't determine size of existing cache value %v", err)
			}

			// Account for that element's removal in evicted and cache size.
			c.size -= es
			room = c.capacity - c.size
...
func (c *Cache) Put(key interface{}, value cache.Value) error {
	vs, err := value.Size()
	if err != nil {
		return fmt.Errorf("couldn't determine size of cache value: %v", err)
	}
	if vs > c.capacity {
		return fmt.Errorf(
			"can't insert entry of size %v into cache with capacity %v",
			vs, c.capacity)
	}
...
func (c *Cache) Get(key interface{}) (cache.Value, error) {
	...
	// When the cache needs to evict a element to make space for another one, it
	// starts eviction from the back, so by moving this element to the front,
	// it's eviction is delayed because it's recently accessed.
	c.ll.MoveToFront(el)
