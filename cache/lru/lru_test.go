...
func TestCacheFailsInsertionSizeBiggerCapacity(t *testing.T) {
	...
	if err == nil {
		t.Fatal("shouldn't be able to insert elements larger than cache")
	}
	assertEqual(t, c.Len(), 0, "")
}
...
// TestConcurrency is a very simple test that checks concurrent access to the
// lru cache. When running the test, "-race" option should be passed to
// "go test" command.
