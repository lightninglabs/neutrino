...
func (h HeaderType) Size() int {
	switch h {
	case Block:
		return BlockHeaderSize
	case RegularFilter:
		return RegularFilterHeaderSize
	default:
		return UnknownHeaderSize
	}
}

// Tip returns the current tip key for the given header type.
// Returns an error if the header type is unknown.
func (h HeaderType) Tip() ([]byte, error) {
	switch h {
	case Block:
	...
	default:
		return nil, fmt.Errorf("UnknownHeaderType(%d)", h)
	}
}
