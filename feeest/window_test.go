...
// TestWindowSameHeightDistinctHash confirms a second block at the same
// height with a different hash replaces the first entry rather than sitting
// alongside it. The only way two hashes compete for one height is a reorg
// racing an in-flight Observe, and replacement guarantees the estimator can
// never double-weight that height with orphaned-chain data.
func TestWindowSameHeightDistinctHash(t *testing.T) {
	...
	fork := mkForkSample(7)
	require.True(t, w.add(fork))
	require.Equal(t, 1, w.len())

	got := w.snapshot()
	require.Equal(t, fork.BlockHash, got[0].BlockHash,
		"replacement must keep the most recently observed block")
}
