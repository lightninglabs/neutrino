...
type sampleWindow struct {
	mu sync.RWMutex
	// data is kept sorted in ascending height order.
...
//
// A sample at a height the window already holds under a different hash
// replaces the existing entry: that only happens across a reorg, where an
// in-flight Observe for an orphaned block can land after PruneFrom already
// ran. Replacement keeps the invariant of at most one sample per height, so
// a straggling orphan can never sit alongside (and double-weight) the
// new-chain block at the same height.
