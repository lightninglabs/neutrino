...
type Estimator struct {
	...
	// halfLifeNanos holds the EWMA half-life in nanoseconds, updated
	// atomically so adaptive logic never blocks estimate queries. The
	// atomic.Int64 type (rather than raw atomic calls on a plain int64)
	// guarantees the required 8-byte alignment on 32-bit platforms.
	halfLifeNanos atomic.Int64

	// lastAdaptHeight is the highest sample height for which the adaptive
	// half-life logic has already run. Adapting at most once per new
	// block sample keeps repeated Estimate calls over the same window
	// from ratcheting the half-life up or down; the compare-and-swap in
	// maybeAdaptHalfLife enforces that even under concurrent queries.
	lastAdaptHeight atomic.Uint32
...
// of variation has changed between the newer half of the sample window and
// the older half. We need at least 2×minBlocksA samples so both halves carry
// a meaningful population; below that threshold the half-life is left
// unchanged.
//
...
//
// The split is by sample count, not by wall-clock midpoint: a time-based
// split shrinks the recent bucket as the half-life shrinks, so once the
// half-life bottomed out the recent bucket could never repopulate and all
// future adaptation, including the recovery branch, froze until restart.
//
...
// halving each call and ratchet the half-life straight to its clamp. The
// compare-and-swap on lastAdaptHeight guarantees a single winner even when
// concurrent Estimate calls race on the same new sample.
func (e *Estimator) maybeAdaptHalfLife(usable []feedb.FeeSample) {
	...
	for {
		last := e.lastAdaptHeight.Load()
		if newest <= last {
			return
		}
		if e.lastAdaptHeight.CompareAndSwap(last, newest) {
			break
		}
	}
	...
	// usable is height-ordered, so the two halves contrast the newer
	// slice of the window against the older one. The 2×minBlocksA gate
	// above guarantees both halves hold at least minBlocksA samples.
	mid := len(usable) / 2
	prior, recent := usable[:mid], usable[mid:]
