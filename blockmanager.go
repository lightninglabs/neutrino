...
func newBlockManager(cfg *blockManagerCfg,
...
func (c *checkpointedCFHeadersQuery) handleResponse(req, resp wire.Message,
	...
	// checkpoint we got earlier, so we should always return true so that
	// the peer looking for the answer to this query can move on to the
	// next query.  We still have to check that these headers are next
	// before we write them; otherwise, we cache them if they're too far
	// ahead, or discard them if we don't need them.
	select {
	case c.headerChan <- r:
...
func (b *blockManager) rollBackToHeight(height uint32) (*headerfs.BlockStamp, error) {
	...
	for uint32(bs.Height) > height {
		header, _, err := b.cfg.BlockHeaders.FetchHeader(&bs.Hash)
		if err != nil {
			return nil, err
		}
